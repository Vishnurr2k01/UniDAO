...
function Notifications() {
  ...
  return (
    <div className='text-white m-12'>
        <h1 className='text-center text-5xl font-bold '>Notifications from uniDAO for you</h1>
        {
            notification.length===0?<div className='text-black bg-[#00df9a] w-fit py-2 px-6 rounded-full text-xl font-bold' onClick={optin}>Opt in</div>:null
        }
        {notification.filter((item)=>item.app==="uniDAO").map((item)=>{
            return(
