    ...
    contract UniDAO is ReentrancyGuard, AccessControl,ERC2771Recipient {
        ...
        constructor(address[] memory addr,address _trustedForwarder)   {
        ...
        function createProposal(
            ...
            emit Action(_msgSender(), ADMIN_ROLE, "Proposal raised");
        }
            ...
            AdminOnly
        {
            ...
            AdminVotes[_msgSender()].push(_proposalId);

            emit Action(_msgSender(), ADMIN_ROLE, "voted");
        }
            ...
            VoterOnly
        {
            ...
            uint256 _power = enrolled[_msgSender()].power;
            proposal.reactions++;
            if (choosen == 1) proposal.upvotes += _power;
            else proposal.downvotes += _power;

            VoterVotes[_msgSender()].push(_proposalId);

            VotedOn[proposal.proposalId].push(
                VotedStruct(payable(_msgSender()), block.timestamp, choosen, _power)
            );

            emit Action(_msgSender(), VOTER_ROLE, "voted");
        }
            ...
            private
        {
            ...
            if (keccak256(abi.encodePacked(adm)) == keccak256("admin")) {
                uint256[] memory tempVotes = AdminVotes[_msgSender()];
                ...
                uint256[] memory tempVotes = VoterVotes[_msgSender()];
                for (uint256 votes = 0; votes < tempVotes.length; votes++) {
                    if (proposal.proposalId == tempVotes[votes]) {
                        revert("You have already voted");
                    }
